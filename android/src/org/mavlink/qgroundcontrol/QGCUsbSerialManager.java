...
public class QGCUsbSerialManager {
    ...
    private static int getOrCreateResourceId(final int physicalDeviceId, final int portIndex) {
        ...
        final int candidateId = nextResourceId.getAndUpdate(v -> (v >= Integer.MAX_VALUE) ? 1 : v + 1);
    ...
    static int getOrCreateResourceIdForTesting(final int physicalDeviceId, final int portIndex) {
        return getOrCreateResourceId(physicalDeviceId, portIndex);
    }
     ...
     * Collects stale IDs first to avoid nested mutation of concurrent maps during iteration.
     *
