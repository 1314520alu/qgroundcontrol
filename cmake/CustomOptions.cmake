...
set(QGC_CUSTOM_DIR
    "custom"
    CACHE STRING "Custom build overlay directory, relative to the source root"
)
...
set(QGC_APP_NAME
    "米多地面站"
    CACHE STRING "Application name" FORCE
)
string(TIMESTAMP _copyright_year "%Y")
set(QGC_APP_COPYRIGHT
    "Copyright (c) ${_copyright_year} 米多地面站. All rights reserved."
    CACHE STRING "Copyright notice" FORCE
)
...
set(QGC_PROJECT_TARGET_NAME
    "QGroundControl"
    CACHE STRING "CMake project/target name" FORCE
)

# Settings version - increment to clear stored settings on next boot after incompatible changes
set(QGC_SETTINGS_VERSION
    "9"
...
option(QGC_SPLIT_DWARF
       "Use -gsplit-dwarf + --gdb-index for faster Debug links (Linux/Android ELF only; marginal win with mold)" OFF
)
...
set(QGC_LINK_PARALLEL_LEVEL
    2
    CACHE STRING "Maximum parallel link jobs (prevents OOM during LTO)"
)
...
set(QGC_GST_DOWNLOAD_TIMEOUT
    ""
    CACHE STRING "GStreamer SDK download wall-clock timeout (seconds, default 1200)"
)
set(QGC_GST_DOWNLOAD_INACTIVITY_TIMEOUT
    ""
    CACHE STRING "GStreamer SDK download inactivity timeout (seconds, default 60)"
)

# Coverage thresholds
set(QGC_COVERAGE_LINE_THRESHOLD
    30
    CACHE STRING "Minimum line coverage percentage"
)
set(QGC_COVERAGE_BRANCH_THRESHOLD
    20
    CACHE STRING "Minimum branch coverage percentage"
)

# Valgrind options
set(QGC_VALGRIND_TIMEOUT_MULTIPLIER
    20
    CACHE STRING "Timeout multiplier for Valgrind"
)
...
# Android Platform
# ----------------------------------------------------------------------------
set(QGC_QT_ANDROID_COMPILE_SDK_VERSION
    "${QGC_CONFIG_ANDROID_PLATFORM}"
    CACHE STRING "Android compile SDK version"
)
set(QGC_QT_ANDROID_TARGET_SDK_VERSION
    "${QGC_CONFIG_ANDROID_PLATFORM}"
    CACHE STRING "Android target SDK version"
)
set(QGC_QT_ANDROID_MIN_SDK_VERSION
    "${QGC_CONFIG_ANDROID_MIN_SDK}"
    CACHE STRING "Android minimum SDK version"
)
set(QGC_ANDROID_PACKAGE_NAME
    "${QGC_PACKAGE_NAME}"
    CACHE STRING "Android package identifier"
)
set(QGC_ANDROID_PACKAGE_SOURCE_DIR
    "${CMAKE_SOURCE_DIR}/android"
    CACHE PATH "Android package source directory"
)
set(QT_ANDROID_DEPLOYMENT_TYPE
    ""
    CACHE STRING "Android deployment type (empty or Release)"
)
...
# macOS Platform
# ----------------------------------------------------------------------------
set(QGC_MACOS_PLIST_PATH
    "${CMAKE_SOURCE_DIR}/deploy/macos/MacOSXBundleInfo.plist.in"
    CACHE FILEPATH "macOS Info.plist template path"
)
set(QGC_MACOS_BUNDLE_ID
    "${QGC_PACKAGE_NAME}"
    CACHE STRING "macOS bundle identifier"
)
set(QGC_MACOS_ICON_PATH
    "${CMAKE_SOURCE_DIR}/deploy/macos/qgroundcontrol.icns"
    CACHE FILEPATH "macOS application icon path"
)
set(QGC_MACOS_ENTITLEMENTS_PATH
    "${CMAKE_SOURCE_DIR}/deploy/macos/qgroundcontrol.entitlements"
    CACHE FILEPATH "macOS entitlements file path"
)
...
# iOS Platform
# ----------------------------------------------------------------------------
set(QGC_IOS_DEPLOYMENT_TARGET
    "${QGC_CONFIG_IOS_DEPLOYMENT_TARGET}"
    CACHE STRING "iOS minimum deployment target"
)
set(QGC_IOS_TARGETED_DEVICE_FAMILY
    "1,2"
    CACHE STRING "iOS targeted device family (1=iPhone, 2=iPad)"
)
...
option(QGC_RUN_APPIMAGELINT "Run the appimagelint distro-compatibility check after AppImage creation"
       ${_qgc_appimagelint_default}
)
set(QGC_APPIMAGE_ICON_256_PATH
    "${CMAKE_SOURCE_DIR}/deploy/linux/QGroundControl_256.png"
    CACHE FILEPATH "AppImage 256x256 icon path"
)
set(QGC_APPIMAGE_ICON_SCALABLE_PATH
    "${CMAKE_SOURCE_DIR}/deploy/linux/QGroundControl.svg"
    CACHE FILEPATH "AppImage SVG icon path"
)
set(QGC_APPIMAGE_APPRUN_PATH
    "${CMAKE_SOURCE_DIR}/deploy/linux/AppRun"
    CACHE FILEPATH "AppImage AppRun script path"
)
set(QGC_APPIMAGE_DESKTOP_ENTRY_PATH
    "${CMAKE_SOURCE_DIR}/deploy/linux/org.mavlink.qgroundcontrol.desktop.in"
    CACHE FILEPATH "AppImage desktop entry path"
)
set(QGC_APPIMAGE_METADATA_PATH
    "${CMAKE_SOURCE_DIR}/deploy/linux/org.mavlink.qgroundcontrol.appdata.xml.in"
    CACHE FILEPATH "AppImage metadata path"
)
set(QGC_APPIMAGE_APPDATA_DEVELOPER
    "qgroundcontrol"
    CACHE STRING "AppImage developer name"
)
...
set(QGC_CPACK_GENERATOR
    "${_qgc_cpack_default}"
    CACHE STRING "Optional CPack generator for the qgc-package target (alongside the default installer)"
)
...
# Windows Platform
# ----------------------------------------------------------------------------
set(QGC_WINDOWS_INSTALL_HEADER_PATH
    "${CMAKE_SOURCE_DIR}/deploy/windows/installheader.bmp"
    CACHE FILEPATH "Windows installer header image"
)
set(QGC_WINDOWS_ICON_PATH
    "${CMAKE_SOURCE_DIR}/deploy/windows/WindowsQGC.ico"
    CACHE FILEPATH "Windows application icon"
)
set(QGC_WINDOWS_RESOURCE_FILE_PATH
    "${CMAKE_SOURCE_DIR}/deploy/windows/QGroundControl.rc"
    CACHE FILEPATH "Windows resource file"
)
...
set(QGC_QT_MINIMUM_VERSION
    "${QGC_CONFIG_QT_MINIMUM_VERSION}"
    CACHE STRING "Minimum supported Qt version"
)
set(QGC_QT_MAXIMUM_VERSION
    "${QGC_CONFIG_QT_VERSION}"
    CACHE STRING "Maximum supported Qt version"
)

set(QT_QML_OUTPUT_DIRECTORY
    "${CMAKE_BINARY_DIR}/qml"
    CACHE PATH "QML output directory"
)
set(QML_IMPORT_PATH
