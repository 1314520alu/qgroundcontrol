...
class MavlinkCameraControlInterface : public FactGroup
{
    ...
    Q_PROPERTY(bool hasExposureAuto READ hasExposureAuto NOTIFY infoChanged)
    ...
    Q_PROPERTY(PhotoCaptureMode photoCaptureMode READ photoCaptureMode WRITE setPhotoCaptureMode NOTIFY
    ...
    // These properties are used to determine what controls to show in the UI and are based on both the camera
    // capabilities as well as the video manager status. They are not necessarily directly related to the MAVLink camera
    // capabilities.
    ...
    // These are "virtual" states which take into account both the mavlink camera capabilities as well as the gstreamer
    // ability to locally record video as well as to do a photo grab from the video stream. These are what the UI should
    // use to determine what options to present to the user and are updated based on both the camera info as well as the
...
public:
    ...
    virtual bool hasExposureAuto() const { return false; }
    ...
    virtual bool incomingParameter(
        Fact* pFact, QVariant& newValue) = 0;   ///< Allow controller to modify or invalidate incoming parameter
    virtual bool validateParameter(
        Fact* pFact, QVariant& newValue) = 0;   ///< Allow controller to modify or invalidate parameter change
