...
static void _requestFovOnZoom_Handler(void* user, MAV_RESULT result,
                                      ...
                                      const mavlink_message_t& message)
{
    ...
    if (result != MAV_RESULT_ACCEPTED) {
        qCDebug(CameraManagerLog) << "CAMERA_FOV_STATUS request failed, result:" << result << "failure:" << failureCode;
        return;
    }
...
QGCCameraManager::CameraStruct::CameraStruct(QGCCameraManager* manager_, uint8_t compID_, Vehicle* vehicle_)
    : compID(compID_), vehicle(vehicle_), manager(manager_)
{
      ...
      _topotekCameraControl(new TopotekTq10CameraControl(vehicle, _topotekClient, this))
{
    ...
    if (_vehicle) {
        (void) connect(_vehicle, &Vehicle::initialConnectComplete, this, &QGCCameraManager::_initialConnectCompleted,
                       Qt::UniqueConnection);
        (void) connect(_vehicle, &Vehicle::mavlinkMessageReceived, this, &QGCCameraManager::_mavlinkMessageReceived);
    }
    (void) connect(MultiVehicleManager::instance(), &MultiVehicleManager::parameterReadyVehicleAvailableChanged, this,
                   &QGCCameraManager::_vehicleReady);
...
void QGCCameraManager::_vehicleReady(bool ready)
{
    ...
    (void) connect(JoystickManager::instance(), &JoystickManager::activeJoystickChanged, this,
                   &QGCCameraManager::_activeJoystickChanged, Qt::UniqueConnection);
}
...
void QGCCameraManager::_handleHeartbeat(const mavlink_message_t& message)
{
    ...
    if (pInfo->lastHeartbeat.elapsed() > kSilentTimeoutMs) {
        qCDebug(CameraManagerLog) << "Camera" << QGCMAVLink::compIdToString(message.compid)
...
MavlinkCameraControlInterface* QGCCameraManager::currentCameraInstance()
{
    ...
    if ((_currentCameraIndex < _cameras.count()) && !_cameras.isEmpty()) {
        MavlinkCameraControlInterface* pCamera =
            qobject_cast<MavlinkCameraControlInterface*>(_cameras[_currentCameraIndex]);
        return pCamera;
    }
    return nullptr;
}
...
void QGCCameraManager::_addCameraControlToLists(MavlinkCameraControlInterface* cameraControl)
{
    ...
    if ((_cameras.count() == 2) && (_cameras[0] == _simulatedCameraControl)) {
        ...
        if (keepSimulatedForManualStream) {
            qCInfo(CameraManagerLog) << "Keeping simulated camera alongside real camera (manual video stream active)";
        } else {
...
void QGCCameraManager::_ensureSimulatedCameraForLocalRecord()
{
    ...
    const bool curCanShowControls =
        cur && (cur->capturesVideo() || cur->capturesPhotos() || cur->hasTracking() || cur->hasVideoStream());
    if (!curCanShowControls && (simIdx >= 0) && (_currentCameraIndex != simIdx)) {
        qCInfo(CameraManagerLog)
            << "Selecting simulated camera for PhotoVideoControl (current camera has no capture UI)";
...
void QGCCameraManager::_handleCameraInfo(const mavlink_message_t& message)
{
    const QString sCompID = QString::number(message.compid);
    if (!_cameraInfoRequest.contains(sCompID)) {
        qCDebug(CameraManagerLog) << "Ignoring - Camera info not requested for component"
                                  << QGCMAVLink::compIdToString(message.compid);
        return;
    }
    if (_cameraInfoRequest[sCompID]->infoReceived) {
        qCDebug(CameraManagerLog) << "Ignoring - Already received camera info for component"
                                  << QGCMAVLink::compIdToString(message.compid);
        return;
    }
    ...
    MavlinkCameraControlInterface* pCamera =
        _vehicle->firmwarePlugin()->createCameraControl(&info, _vehicle, message.compid, this);
    if (pCamera) {
        ...
        qCDebug(CameraManagerLog) << "Success for compId" << QGCMAVLink::compIdToString(message.compid)
                                  << "- reset retry counter";
    }
...
void QGCCameraManager::_handleCameraSettings(const mavlink_message_t& message)
{
    ...
    if (pCamera) {
        ...
        if (QThread::currentThread() == thread()) {
            ...
            QMetaObject::invokeMethod(this, "_setCurrentZoomLevel", Qt::QueuedConnection, Q_ARG(int, newZoom));
        }
...
static QGCCameraManager::CameraStruct* _cameraStructFromContext(void* resultHandlerData)
{
    ...
    if (!cameraInfo) {
        qCDebug(CameraManagerLog) << "Camera info request callback for removed camera. compId"
                                  << QGCMAVLink::compIdToString(context->compID);
    }
    return cameraInfo;
}
                                                   ...
                                                   Vehicle::MavCmdResultFailureCode_t failureCode)
{
    ...
    if (ack.result != MAV_RESULT_ACCEPTED) {
        qCDebug(CameraManagerLog) << "MAV_CMD_REQUEST_CAMERA_INFORMATION failed. compId"
...
static void _requestCameraInfoMessageResultHandler(void* resultHandlerData, MAV_RESULT result,
                                                   Vehicle::RequestMessageResultHandlerFailureCode_t failureCode,
                                                   [[maybe_unused]] const mavlink_message_t& message)
{
    QGCCameraManager::CameraStruct* cameraInfo = _cameraStructFromContext(resultHandlerData);
    if (!cameraInfo) {
        return;
    ...
    if (result != MAV_RESULT_ACCEPTED) {
        qCDebug(CameraManagerLog) << "MAV_CMD_REQUEST_MESSAGE:MAVLINK_MSG_ID_CAMERA_INFORMATION failed. compId"
                                  << QGCMAVLink::compIdToString(cameraInfo->compID)
                                  << "Result:" << QGCMAVLink::mavResultToString(result) << "FailureCode:"
                                  << Vehicle::requestMessageResultHandlerFailureCodeToString(failureCode)
                                  << "retryCount:" << cameraInfo->retryCount;
        _handleCameraInfoRetry(cameraInfo);
    }
}
...
static void _requestCameraInfoHelper(QGCCameraManager* manager, QGCCameraManager::CameraStruct* pInfo)
{
    ...
    if (pInfo->retryCount >= kMaxRetryCount) {
        qCDebug(CameraManagerLog) << "Giving up requesting camera info after" << pInfo->retryCount
                                  << "attempts for compId" << QGCMAVLink::compIdToString(pInfo->compID);
        return;
    }

    // Alternate between REQUEST_MESSAGE and REQUEST_CAMERA_INFORMATION
    if ((pInfo->retryCount % 2) == 0) {
        qCDebug(CameraManagerLog) << "Using MAV_CMD_REQUEST_MESSAGE:CAMERA_INFORMATION for compId"
                                  << QGCMAVLink::compIdToString(pInfo->compID);
        manager->vehicle()->requestMessage(_requestCameraInfoMessageResultHandler,
                                           manager->cameraInfoContext(pInfo->compID), pInfo->compID,
                                           MAVLINK_MSG_ID_CAMERA_INFORMATION);
    } else {
        qCDebug(CameraManagerLog) << "Using MAV_CMD_REQUEST_CAMERA_INFORMATION for compId"
                                  << QGCMAVLink::compIdToString(pInfo->compID);
        ...
        pInfo->vehicle->sendMavCommandWithHandler(&ackHandlerInfo, pInfo->compID, MAV_CMD_REQUEST_CAMERA_INFORMATION,
...
static void _handleCameraInfoRetry(QGCCameraManager::CameraStruct* cameraInfo)
{
    ...
    if (!manager) {
        qCDebug(CameraManagerLog) << "manager is unavailable for compId"
                                  << QGCMAVLink::compIdToString(cameraInfo->compID);
        return;
    }
    ...
    if ((cameraInfo->retryCount >= 2) && ((cameraInfo->retryCount % 2) == 0)) {
        ...
        qCDebug(CameraManagerLog) << "Waiting" << delaySeconds << "seconds before retry for compId"
                                  << QGCMAVLink::compIdToString(cameraInfo->compID);
...
void QGCCameraManager::_activeJoystickChanged(Joystick* joystick)
{
    ...
    if (_activeJoystick) {
        ...
        (void) disconnect(_activeJoystick, &Joystick::toggleVideoRecord, this,
    ...
    if (_activeJoystick) {
        ...
        (void) connect(_activeJoystick, &Joystick::startContinuousZoom, this, &QGCCameraManager::_startZoom,
                       Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::stopContinuousZoom, this, &QGCCameraManager::_stopZoom,
                       Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::stepFocus, this, &QGCCameraManager::_stepFocus,
                       Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::startContinuousFocus, this, &QGCCameraManager::_startFocus,
                       Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::stopContinuousFocus, this, &QGCCameraManager::_stopFocus,
                       Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::stepCamera, this, &QGCCameraManager::_stepCamera,
                       Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::stepStream, this, &QGCCameraManager::_stepStream,
                       Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::triggerCamera, this, &QGCCameraManager::_triggerCamera,
                       Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::startVideoRecord, this, &QGCCameraManager::_startVideoRecording,
                       Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::stopVideoRecord, this, &QGCCameraManager::_stopVideoRecording,
                       Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::toggleVideoRecord, this, &QGCCameraManager::_toggleVideoRecording,
                       Qt::UniqueConnection);
    }
}
...
void QGCCameraManager::requestCameraFovForComp(int compId)
{
    ...
    _vehicle->requestMessage(_requestFovOnZoom_Handler, /*user*/ this, compId, MAVLINK_MSG_ID_CAMERA_FOV_STATUS);
}
...
double QGCCameraManager::aspectForComp(int compId) const
{
    ...
    return (it == _aspectByCompId.cend()) ? std::numeric_limits<double>::quiet_NaN() : it.value();
}
...
void QGCCameraManager::_handleCameraFovStatus(const mavlink_message_t& message)
{
    ...
    if (!std::isfinite(vfovDeg) || vfovDeg <= 0.0 || vfovDeg >= 180.0) {
        qCWarning(CameraManagerLog) << "Invalid calculated VFOV:" << vfovDeg << "hfov:" << fov.hfov
                                    << "aspect:" << aspect << "compId:" << message.compid;
        return;
    }
