...
Q_MOC_INCLUDE("SatcomAntennaController.h")
...
public:
    ...
    Q_PROPERTY(SatcomAntennaController* satcomAntennaController READ satcomAntennaController CONSTANT)
...
#endif
    Q_PROPERTY(
        QGCPalette* globalPalette MEMBER _globalPalette CONSTANT)  ///< This palette will always return enabled colors
    ...
    Q_PROPERTY(QGeoCoordinate flightMapPosition READ flightMapPosition WRITE setFlightMapPosition NOTIFY
    ...
    Q_PROPERTY(double flightMapInitialZoom MEMBER _flightMapInitialZoom
                   CONSTANT)  ///< Zoom level to use when either gcs or vehicle shows up for first time
    ...
    Q_PROPERTY(
        qreal zOrderTopMost READ zOrderTopMost CONSTANT)  ///< z order for top most items, toolbar, main window sub view
    Q_PROPERTY(qreal zOrderWidgets READ zOrderWidgets
                   CONSTANT)  ///< z order value to widgets, for example: zoom controls, hud widgetss
    ...
    Q_INVOKABLE void startPX4MockLink(bool sendStatusText, bool enableCamera, bool enableGimbal,
                                      bool enableProximity = false, int videoStreamType = 0);
    ...
    Q_INVOKABLE void startAPMArduCopterMockLink(bool sendStatusText, bool enableCamera, bool enableGimbal,
                                                bool enableProximity = false, bool apmStartFreshParams = false,
                                                int videoStreamType = 0);
    ...
    Q_INVOKABLE QString
    altitudeFrameExtraUnits(AltitudeFrame altFrame);  ///< String shown in the FactTextField.extraUnits ui
    Q_INVOKABLE QString altitudeFrameShortDescription(
        AltitudeFrame altFrame);                      ///< String shown when a user needs to select an altitude frame
    ...
    ///   @param owner          The QML item that owns this dialog. The dialog will be destroyed when this item is
    ///   destroyed.
    ///                             Typically mainWindow should not be used as the owner, instead use a more specific
    ///                             item such as the one that triggered the dialog to ensure proper cleanup.
    ...
    Q_INVOKABLE void showMessageDialog(QObject* owner, const QString& title, const QString& text,
                                       int buttons = kDefaultMessageDialogButtons, QJSValue acceptFunction = QJSValue(),
    ...
    SatcomAntennaController* satcomAntennaController() { return _satcomAntennaController; }
...
signals:
    ...
    void showMessageDialogRequested(QObject* owner, QString title, QString text, int buttons, QJSValue acceptFunction,
...
private:
    ...
    SatcomAntennaController* _satcomAntennaController = nullptr;
