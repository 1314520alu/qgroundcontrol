...
// After a second GCS has requested control and we have given it permission to takeover, we will remove takeover
// permission automatically after this timeout If the second GCS didn't get control
#define REQUEST_OPERATOR_CONTROL_ALLOW_TAKEOVER_TIMEOUT_MSECS 10000

const QString guided_mode_not_supported_by_vehicle = QObject::tr("Guided mode not supported by Vehicle.");

// Standard connected vehicle
Vehicle::Vehicle(LinkInterface* link, int vehicleId, int defaultComponentId, MAV_AUTOPILOT firmwareType,
      ...
      _vehicleFactGroup(this)
{
    ...
    connect(MultiVehicleManager::instance(), &MultiVehicleManager::parameterReadyVehicleAvailableChanged, this,
            &Vehicle::_vehicleParamLoaded);

    _commonInit(link);

    // Set video stream to udp if running ArduSub and Video is disabled
    if (sub() &&
        SettingsManager::instance()->videoSettings()->videoSource()->rawValue() == VideoSettings::videoDisabled) {
      ...
      _vehicleFactGroup(this)
{
    ...
    connect(SettingsManager::instance()->appSettings()->offlineEditingCruiseSpeed(), &Fact::rawValueChanged, this,
            &Vehicle::_offlineCruiseSpeedSettingChanged);
    connect(SettingsManager::instance()->appSettings()->offlineEditingHoverSpeed(), &Fact::rawValueChanged, this,
            &Vehicle::_offlineHoverSpeedSettingChanged);
...
{
    connect(SettingsManager::instance()->appSettings()->offlineEditingFirmwareClass(), &Fact::rawValueChanged, this,
            &Vehicle::_offlineFirmwareTypeSettingChanged);
    connect(SettingsManager::instance()->appSettings()->offlineEditingVehicleClass(), &Fact::rawValueChanged, this,
            &Vehicle::_offlineVehicleTypeSettingChanged);

    _offlineFirmwareTypeSettingChanged(
        SettingsManager::instance()->appSettings()->offlineEditingFirmwareClass()->rawValue());
    _offlineVehicleTypeSettingChanged(
        SettingsManager::instance()->appSettings()->offlineEditingVehicleClass()->rawValue());
}

void Vehicle::stopTrackingFirmwareVehicleTypeChanges(void)
{
    disconnect(SettingsManager::instance()->appSettings()->offlineEditingFirmwareClass(), &Fact::rawValueChanged, this,
               &Vehicle::_offlineFirmwareTypeSettingChanged);
    disconnect(SettingsManager::instance()->appSettings()->offlineEditingVehicleClass(), &Fact::rawValueChanged, this,
               &Vehicle::_offlineVehicleTypeSettingChanged);
}
...
void Vehicle::_commonInit(LinkInterface* link)
{
    ...
    connect(QGCPositionManager::instance(), &QGCPositionManager::gcsPositionChanged, this,
            &Vehicle::_updateDistanceHeadingGCS);
    ...
    connect(_messageIntervalManager, &MessageIntervalManager::mavlinkMsgIntervalsChanged, this,
            &Vehicle::mavlinkMsgIntervalsChanged);
    _terrainQueryCoordinator = new TerrainQueryCoordinator(this);
    connect(this, &Vehicle::coordinateChanged, _terrainQueryCoordinator,
            &TerrainQueryCoordinator::updateAltAboveTerrain);
    ...
    connect(_standardModes, &StandardModes::modesUpdated, this, [this]() { emit flightModeChanged(flightMode()); });

    _parameterManager = new ParameterManager(this);
    connect(_parameterManager, &ParameterManager::parametersReadyChanged, this, &Vehicle::_parametersReady);
    connect(_parameterManager, &ParameterManager::parametersReadyChanged, this,
            [this](bool) { emit hasGripperChanged(); });
    connect(_initialConnectStateMachine, &InitialConnectStateMachine::progressUpdate, this,
            &Vehicle::_gotProgressUpdate);
...
void Vehicle::_mavlinkMessageReceived(LinkInterface* link, mavlink_message_t message)
{
    ...
    switch (message.msgid) {
        ...
        case MAVLINK_MSG_ID_SERVO_OUTPUT_RAW: {
            ...
            const uint16_t rawValues[16] = {
                servoOutputRaw.servo1_raw,  servoOutputRaw.servo2_raw,  servoOutputRaw.servo3_raw,
                servoOutputRaw.servo4_raw,  servoOutputRaw.servo5_raw,  servoOutputRaw.servo6_raw,
                servoOutputRaw.servo7_raw,  servoOutputRaw.servo8_raw,  servoOutputRaw.servo9_raw,
                servoOutputRaw.servo10_raw, servoOutputRaw.servo11_raw, servoOutputRaw.servo12_raw,
                servoOutputRaw.servo13_raw, servoOutputRaw.servo14_raw, servoOutputRaw.servo15_raw,
                servoOutputRaw.servo16_raw};

            for (int servoIndex = 0; servoIndex < _servoOutputRawValues.size() && servoIndex < 16; servoIndex++) {
                _servoOutputRawValues[servoIndex] =
                    (rawValues[servoIndex] == UINT16_MAX) ? -1 : static_cast<int>(rawValues[servoIndex]);
            }
    ...
    // This must be emitted after the vehicle processes the message. This way the vehicle state is up to date when
    // anyone else does processing.
...
void Vehicle::_handleCameraFeedback(const mavlink_message_t& message)
{
    ...
    QGeoCoordinate imageCoordinate((double) feedback.lat / qPow(10.0, 7.0), (double) feedback.lng / qPow(10.0, 7.0),
                                   feedback.alt_msl);
...
void Vehicle::_handleOrbitExecutionStatus(const mavlink_message_t& message)
{
    ...
    QGeoCoordinate newCenter(static_cast<double>(orbitStatus.x) / qPow(10.0, 7.0),
...
void Vehicle::_handleCameraImageCaptured(const mavlink_message_t& message)
{
    ...
    QGeoCoordinate imageCoordinate((double) feedback.lat / qPow(10.0, 7.0), (double) feedback.lon / qPow(10.0, 7.0),
                                   feedback.alt);
    qCDebug(VehicleLog) << "_handleCameraFeedback coord:index" << imageCoordinate << feedback.image_index
                        << feedback.capture_result;
...
void Vehicle::_handleGpsRawInt(mavlink_message_t& message)
{
    ...
    if (gpsRawInt.fix_type >= GPS_FIX_TYPE_3D_FIX) {
        if (!_globalPositionIntMessageAvailable) {
            QGeoCoordinate newPosition(gpsRawInt.lat / (double) 1E7, gpsRawInt.lon / (double) 1E7,
                                       gpsRawInt.alt / 1000.0);
...
void Vehicle::_handleGlobalPositionInt(mavlink_message_t& message)
{
    ...
    QGeoCoordinate newPosition(globalPositionInt.lat / (double) 1E7, globalPositionInt.lon / (double) 1E7,
...
void Vehicle::_handleHighLatency(mavlink_message_t& message)
{
    ...
    if (_base_mode != 0 || _custom_mode != 0) {
        // Vehicle is initialized with _base_mode=0 and _custom_mode=0. Don't pass this to flightMode() since it will
        // complain about bad modes while unit testing.
    ...
    } coordinate{highLatency.latitude / (double) 1E7, highLatency.longitude / (double) 1E7,
...
void Vehicle::_setCapabilities(uint64_t capabilityBits)
{
    ...
    qCDebug(VehicleLog) << QString("Vehicle %1 Mavlink 2.0")
                               .arg(_capabilityBits & MAV_PROTOCOL_CAPABILITY_MAVLINK2 ? supports : doesNotSupport);
    qCDebug(VehicleLog) << QString("Vehicle %1 MISSION_ITEM_INT")
                               .arg(_capabilityBits & MAV_PROTOCOL_CAPABILITY_MISSION_INT ? supports : doesNotSupport);
    qCDebug(VehicleLog) << QString("Vehicle %1 MISSION_COMMAND_INT")
                               .arg(_capabilityBits & MAV_PROTOCOL_CAPABILITY_COMMAND_INT ? supports : doesNotSupport);
    qCDebug(VehicleLog) << QString("Vehicle %1 GeoFence")
                               .arg(_capabilityBits & MAV_PROTOCOL_CAPABILITY_MISSION_FENCE ? supports
                                                                                            : doesNotSupport);
    qCDebug(VehicleLog) << QString("Vehicle %1 RallyPoints")
                               .arg(_capabilityBits & MAV_PROTOCOL_CAPABILITY_MISSION_RALLY ? supports
                                                                                            : doesNotSupport);
    qCDebug(VehicleLog) << QString("Vehicle %1 Terrain")
                               .arg(_capabilityBits & MAV_PROTOCOL_CAPABILITY_TERRAIN ? supports : doesNotSupport);
}
...
QString Vehicle::vehicleUIDStr()
{
    ...
    uid = uid.asprintf("%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X", pUid[0] & 0xff, pUid[1] & 0xff, pUid[2] & 0xff,
                       pUid[3] & 0xff, pUid[4] & 0xff, pUid[5] & 0xff, pUid[6] & 0xff, pUid[7] & 0xff);
    return uid;
}
...
void Vehicle::_handleSysStatus(mavlink_message_t& message)
{
    ...
    bool newAllSensorsHealthy = (sysStatus.onboard_control_sensors_enabled &
                                 sysStatus.onboard_control_sensors_health) == sysStatus.onboard_control_sensors_enabled;
    ...
    // ArduPilot firmare has a strange case when ARMING_REQUIRE=0. This means the vehicle is always armed but the motors
    // are not really powered up until the safety button is pressed. Because of this we can't depend on the heartbeat to
    // tell us the true armed (and dangerous) state. We must instead rely on SYS_STATUS telling us that the motors are
    // enabled.
...
void Vehicle::_handleHomePosition(mavlink_message_t& message)
{
    ...
    QGeoCoordinate newHomePosition(homePos.latitude / 10000000.0, homePos.longitude / 10000000.0,
...
void Vehicle::_handlePing(LinkInterface* link, mavlink_message_t& message)
{
    ...
    if ((ping.target_system == 0) && (ping.target_component == 0)) {
        ...
        mavlink_msg_ping_pack_chan(static_cast<uint8_t>(MAVLinkProtocol::instance()->getSystemId()),
                                   ...
                                   sharedLink->mavlinkChannel(), &msg, ping.time_usec, ping.seq, message.sysid,
                                   message.compid);
        sendMessageOnLinkThreadSafe(link, msg);
    }
}

void Vehicle::setActuatorsMetadata([[maybe_unused]] uint8_t compid, const QString& metadataJsonFileName,
...
void Vehicle::_handleHeartbeat(mavlink_message_t& message)
{
    ...
    if (heartbeat.base_mode != _base_mode || heartbeat.custom_mode != _custom_mode) {
        QString previousFlightMode;
        if (_base_mode != 0 || _custom_mode != 0) {
            // Vehicle is initialized with _base_mode=0 and _custom_mode=0. Don't pass this to flightMode() since it
            // will complain about bad modes while unit testing.
            previousFlightMode = flightMode();
        }
...
int Vehicle::motorCount()
{
    ...
    if (_vehicleType == MAV_TYPE_SUBMARINE) {
        ...
        if (!frameConfig) {
            qCWarning(VehicleLog) << "FRAME_CONFIG parameter not found";
        } else {
...
{
    sendMavCommand(_defaultComponentId, MAV_CMD_COMPONENT_ARM_DISARM,
...
void Vehicle::setFlightMode(const QString& flightMode)
{
    ...
    if (setFlightModeCustom(flightMode, &base_mode, &custom_mode)) {
        ...
        if (_firmwarePlugin->MAV_CMD_DO_SET_MODE_is_supported()) {
            sendMavCommand(defaultComponentId(), MAV_CMD_DO_SET_MODE,
                           true,  // show error if fails
                           MAV_MODE_FLAG_CUSTOM_MODE_ENABLED, custom_mode);
        } else {
            mavlink_message_t msg;
            mavlink_msg_set_mode_pack_chan(MAVLinkProtocol::instance()->getSystemId(),
                                           MAVLinkProtocol::getComponentId(), sharedLink->mavlinkChannel(), &msg, id(),
                                           newBaseMode, custom_mode);
...
void Vehicle::_sendMessageMultipleNext()
{
    if (_nextSendMessageMultipleIndex < _sendMessageMultipleList.count()) {
        ...
        SharedLinkInterfacePtr sharedLink = vehicleLinkManager()->primaryLink().lock();
        if (sharedLink) {
            sendMessageOnLinkThreadSafe(sharedLink.get(),
                                        _sendMessageMultipleList[_nextSendMessageMultipleIndex].message);
        }
...
void Vehicle::_sendQGCTimeToVehicle()
{
    ...
    cmd.time_boot_ms = 0;
    mavlink_msg_system_time_encode_chan(MAVLinkProtocol::instance()->getSystemId(), MAVLinkProtocol::getComponentId(),
                                        sharedLink->mavlinkChannel(), &msg, &cmd);
...
{
    // The following if statement prevents the virtualTabletJoystick from sending values if the standard joystick is
    // enabled
    ...
    if (!joystickEnabled) {
        sendJoystickDataThreadSafe(static_cast<float>(roll), static_cast<float>(pitch), static_cast<float>(yaw),
                                   static_cast<float>(thrust), 0, 0,         // buttons
...
bool Vehicle::guidedModeGotoLocation(const QGeoCoordinate& gotoCoord, double forwardFlightLoiterRadius)
{
    ...
    double maxDistance =
        SettingsManager::instance()->flyViewSettings()->maxGoToLocationDistance()->rawValue().toDouble();
...
void Vehicle::guidedModeOrbit(const QGeoCoordinate& centerCoord, double radius, double amslAltitude)
{
    ...
    if (capabilityBits() & MAV_PROTOCOL_CAPABILITY_COMMAND_INT) {
        sendMavCommandInt(
            defaultComponentId(), MAV_CMD_DO_ORBIT, MAV_FRAME_GLOBAL,
            true,                                               // show error if fails
        ...
        sendMavCommand(
            defaultComponentId(), MAV_CMD_DO_ORBIT,
            true,                                               // show error if fails
            static_cast<float>(radius),
            static_cast<float>(qQNaN()),                        // Use default velocity
            static_cast<float>(ORBIT_YAW_BEHAVIOUR_UNCHANGED),  // Use current or vehicle default yaw behavior
            static_cast<float>(qQNaN()),                        // Use vehicle default num of orbits behavior
            static_cast<float>(centerCoord.latitude()), static_cast<float>(centerCoord.longitude()),
...
void Vehicle::guidedModeROI(const QGeoCoordinate& centerCoord)
{
    ...
    if (px4Firmware()) {
        ...
        _terrainQueryCoordinator->sendROICommand(centerCoord, MAV_FRAME_GLOBAL_RELATIVE_ALT,
...
void Vehicle::stopGuidedModeROI()
{
    ...
    if (capabilityBits() & MAV_PROTOCOL_CAPABILITY_COMMAND_INT) {
        sendMavCommandInt(defaultComponentId(), MAV_CMD_DO_SET_ROI_NONE, MAV_FRAME_GLOBAL,
                          true,                          // show error if fails
        ...
        sendMavCommand(defaultComponentId(), MAV_CMD_DO_SET_ROI_NONE,
                       true,                          // show error if fails
                       static_cast<float>(qQNaN()),   // Empty
                       static_cast<float>(qQNaN()),   // Empty
                       static_cast<float>(qQNaN()),   // Empty
...
{
    sendMavCommand(defaultComponentId(), MAV_CMD_DO_GO_AROUND,
                   true,  // show error if fails
...
{
    sendMavCommand(defaultComponentId(), MAV_CMD_AIRFRAME_CONFIGURATION,
                   true,   // show error if fails
                   -1.0f,  // all gears
                   0.0f);  // down
}
...
void Vehicle::setCurrentMissionSequence(int seq)
{
    ...
    sendMavCommandWithLambdaFallback(
        [this, seq]() {  // lambda function which uses the deprecated mission_set_current
            ...
            mavlink_msg_mission_set_current_pack_chan(static_cast<uint8_t>(MAVLinkProtocol::instance()->getSystemId()),
                                                      static_cast<uint8_t>(MAVLinkProtocol::getComponentId()),
                                                      sharedLink->mavlinkChannel(), &msg, static_cast<uint8_t>(id()),
                                                      _compID, static_cast<uint16_t>(seq));
            sendMessageOnLinkThreadSafe(sharedLink.get(), msg);
        },
        static_cast<uint8_t>(defaultComponentId()), MAV_CMD_DO_SET_MISSION_CURRENT,
        true,  // showError
        static_cast<uint16_t>(seq));
}

void Vehicle::sendMavCommand(int compId, MAV_CMD command, bool showError, float param1, float param2, float param3,
                             float param4, float param5, float param6, float param7)
{
    _mavCmdQueue->sendCommand(compId, command, showError, param1, param2, param3, param4, param5, param6, param7);
}

void Vehicle::sendMavCommandDelayed(int compId, MAV_CMD command, bool showError, int milliseconds, float param1,
                                    float param2, float param3, float param4, float param5, float param6, float param7)
{
    _mavCmdQueue->sendCommandDelayed(compId, command, showError, milliseconds, param1, param2, param3, param4, param5,
                                     param6, param7);
}

void Vehicle::sendCommand(int compId, int command, bool showError, double param1, double param2, double param3,
                          double param4, double param5, double param6, double param7)
{
    sendMavCommand(compId, static_cast<MAV_CMD>(command), showError, static_cast<float>(param1),
                   static_cast<float>(param2), static_cast<float>(param3), static_cast<float>(param4),
                   static_cast<float>(param5), static_cast<float>(param6), static_cast<float>(param7));
}

void Vehicle::sendMavCommandWithHandler(const MavCmdAckHandlerInfo_t* ackHandlerInfo, int compId, MAV_CMD command,
                                        float param1, float param2, float param3, float param4, float param5,
                                        float param6, float param7)
{
    _mavCmdQueue->sendCommandWithHandler(ackHandlerInfo, compId, command, param1, param2, param3, param4, param5,
                                         param6, param7);
}

void Vehicle::sendMavCommandInt(int compId, MAV_CMD command, MAV_FRAME frame, bool showError, float param1,
                                float param2, float param3, float param4, double param5, double param6, float param7)
{
    _mavCmdQueue->sendCommandInt(compId, command, frame, showError, param1, param2, param3, param4, param5, param6,
                                 param7);
}

void Vehicle::sendMavCommandIntWithHandler(const MavCmdAckHandlerInfo_t* ackHandlerInfo, int compId, MAV_CMD command,
                                           MAV_FRAME frame, float param1, float param2, float param3, float param4,
                                           double param5, double param6, float param7)
{
    _mavCmdQueue->sendCommandIntWithHandler(ackHandlerInfo, compId, command, frame, param1, param2, param3, param4,
                                            param5, param6, param7);
}

void Vehicle::sendMavCommandWithLambdaFallback(std::function<void()> lambda, int compId, MAV_CMD command,
                                               bool showError, float param1, float param2, float param3, float param4,
                                               float param5, float param6, float param7)
{
    _mavCmdQueue->sendCommandWithLambdaFallback(std::move(lambda), compId, command, showError, param1, param2, param3,
                                                param4, param5, param6, param7);
}

void Vehicle::sendMavCommandIntWithLambdaFallback(std::function<void()> lambda, int compId, MAV_CMD command,
                                                  MAV_FRAME frame, bool showError, float param1, float param2,
                                                  float param3, float param4, double param5, double param6,
                                                  float param7)
{
    _mavCmdQueue->sendCommandIntWithLambdaFallback(std::move(lambda), compId, command, frame, showError, param1, param2,
...
void Vehicle::_handleCommandAck(mavlink_message_t& message)
{
    ...
    QString logMsg = QStringLiteral("_handleCommandAck command(%1) result(%2)")
                         .arg(rawCommandName)
                         .arg(QGCMAVLink::mavResultToString(static_cast<MAV_RESULT>(ack.result)));
...
void Vehicle::requestMessage(RequestMessageResultHandler resultHandler, void* resultHandlerData, int compId,
                             int messageId, float param1, float param2, float param3, float param4, float param5)
{
    _reqMsgCoord->requestMessage(resultHandler, resultHandlerData, compId, messageId, param1, param2, param3, param4,
                                 param5);
}
...
void Vehicle::_rebootCommandResultHandler(void* resultHandlerData, int /*compId*/, const mavlink_command_ack_t& ack,
...
void Vehicle::startCalibration(QGCMAVLink::CalibrationType calType)
{
    ...
    // We can't use sendMavCommand here since we have no idea how long it will be before the command returns a result.
    // This in turn causes the retry logic to break down.
    mavlink_message_t msg;
    mavlink_msg_command_long_pack_chan(MAVLinkProtocol::instance()->getSystemId(), MAVLinkProtocol::getComponentId(),
                                       sharedLink->mavlinkChannel(), &msg, id(),
...
{
    sendMavCommand(_defaultComponentId, MAV_CMD_DO_MOTOR_TEST, showError, motor, MOTOR_TEST_THROTTLE_PERCENT, percent,
                   timeoutSecs, 0, MOTOR_TEST_ORDER_BOARD);
}
...
void Vehicle::setVtolInFwdFlight(bool vtolInFwdFlight)
{
    if (_vtolInFwdFlight != vtolInFwdFlight) {
        sendMavCommand(_defaultComponentId, MAV_CMD_DO_VTOL_TRANSITION,
                       true,                                                     // show errors
...
void Vehicle::_ackMavlinkLogData(uint16_t sequence)
{
    ...
    mavlink_msg_logging_ack_encode_chan(MAVLinkProtocol::instance()->getSystemId(), MAVLinkProtocol::getComponentId(),
                                        sharedLink->mavlinkChannel(), &msg, &ack);
    sendMessageOnLinkThreadSafe(sharedLink.get(), msg);
}
...
void Vehicle::_handleMavlinkLoggingData(mavlink_message_t& message)
{
    ...
    if (static_cast<size_t>(log.length) > sizeof(log.data)) {
        ...
        emit mavlinkLogData(this, log.target_system, log.target_component, log.sequence, log.first_message_offset,
...
void Vehicle::_updateHeadingToNextWP()
{
    ...
    if (llist.size() > currentIndex && currentIndex != -1 && llist[currentIndex]->coordinate().longitude() != 0.0 &&
...
{
    const bool setHomeCmdSupported =
        firmwarePlugin()->supportedMissionCommands(vehicleClass()).contains(MAV_CMD_DO_SET_HOME);
    const bool updateHomeActivated =
        SettingsManager::instance()->flyViewSettings()->updateHomePosition()->rawValue().toBool();
...
float Vehicle::gcsLinkQualityFromLossPercent(float lossPercent)
{
    if (!std::isfinite(lossPercent)) {
        return 0.0f;
    }
    return qBound(0.0f, 100.0f - lossPercent, 100.0f);
}

void Vehicle::_mavlinkMessageStatus(int uasId, uint64_t totalSent, uint64_t totalReceived, uint64_t totalLoss,
                                    float lossPercent)
{
    if (uasId == _systemID) {
...
{
    sendMavCommand(defaultComponentId(), MAV_CMD_SET_MESSAGE_INTERVAL,
                   true,  // show error
                   messageId, rate);
}
...
void Vehicle::sendParamMapRC(const QString& paramName, double scale, double centerValue, int tuningID, double minValue,
                             double maxValue)
{
    SharedLinkInterfacePtr sharedLink = vehicleLinkManager()->primaryLink().lock();
    if (!sharedLink) {
    ...
    mavlink_msg_param_map_rc_pack_chan(
        ...
        static_cast<uint8_t>(MAVLinkProtocol::getComponentId()), sharedLink->mavlinkChannel(), &message, _systemID,
        MAV_COMP_ID_AUTOPILOT1, param_id_cstr,
...
void Vehicle::clearAllParamMapRC(void)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        mavlink_msg_param_map_rc_pack_chan(static_cast<uint8_t>(MAVLinkProtocol::instance()->getSystemId()),
                                           static_cast<uint8_t>(MAVLinkProtocol::getComponentId()),
                                           sharedLink->mavlinkChannel(), &message, _systemID, MAV_COMP_ID_AUTOPILOT1,
                                           param_id_cstr,
...
void Vehicle::sendJoystickDataThreadSafe(float roll, float pitch, float yaw, float thrust, quint16 buttons,
                                         quint16 buttons2, float pitchExtension, float rollExtension, float aux1,
                                         float aux2, float aux3, float aux4, float aux5, float aux6)
{
    SharedLinkInterfacePtr sharedLink = vehicleLinkManager()->primaryLink().lock();
    if (!sharedLink) {
    ...
    mavlink_msg_manual_control_pack_chan(
        static_cast<uint8_t>(MAVLinkProtocol::instance()->getSystemId()),
        static_cast<uint8_t>(MAVLinkProtocol::getComponentId()), sharedLink->mavlinkChannel(), &message,
        static_cast<uint8_t>(_systemID), static_cast<int16_t>(newPitchCommand), static_cast<int16_t>(newRollCommand),
        static_cast<int16_t>(newThrustCommand), static_cast<int16_t>(newYawCommand), buttons, buttons2, extensions,
        outgoingExtensionValues[0], outgoingExtensionValues[1], outgoingExtensionValues[2], outgoingExtensionValues[3],
        outgoingExtensionValues[4], outgoingExtensionValues[5], outgoingExtensionValues[6], outgoingExtensionValues[7]);
    sendMessageOnLinkThreadSafe(sharedLink.get(), message);
}
                                                  ...
                                                  bool useRcOverride)
{
    ...
    if (!useRcOverride || !anyEnabledChannel) {
        ...
        mavlink_msg_rc_channels_override_pack_chan(
            static_cast<uint8_t>(MAVLinkProtocol::instance()->getSystemId()),
            static_cast<uint8_t>(MAVLinkProtocol::getComponentId()), sharedLink->mavlinkChannel(), &releaseMessage,
            static_cast<uint8_t>(_systemID), static_cast<uint8_t>(_defaultComponentId),
...
{
    sendMavCommand(_defaultComponentId, MAV_CMD_DO_DIGICAM_CONTROL,
                   true,                // show errors
...
{
    sendMavCommand(_defaultComponentId, MAV_CMD_DO_GRIPPER,
                   true,            // Show errors
...
void Vehicle::setEstimatorOrigin(const QGeoCoordinate& centerCoord)
{
    ...
    sendMavCommandIntWithLambdaFallback(
        ...
        defaultComponentId(), MAV_CMD_DO_SET_GLOBAL_ORIGIN, MAV_FRAME_GLOBAL,
        false,                                      // showError
...
void Vehicle::setEstimatorOrigin_SET_GPS_GLOBAL_ORIGIN(const QGeoCoordinate& centerCoord)
{
    ...
    mavlink_message_t msg;
    mavlink_msg_set_gps_global_origin_pack_chan(MAVLinkProtocol::instance()->getSystemId(),
                                                MAVLinkProtocol::getComponentId(), sharedLink->mavlinkChannel(), &msg,
                                                id(), centerCoord.latitude() * 1e7, centerCoord.longitude() * 1e7,
                                                centerCoord.altitude() * 1e3, static_cast<float>(qQNaN()));
    sendMessageOnLinkThreadSafe(sharedLink.get(), msg);
}

void Vehicle::pairRX(int rxType, int rxSubType)
{
    sendMavCommand(_defaultComponentId, MAV_CMD_START_RX_PAIR, true, rxType, rxSubType);
}
...
void Vehicle::requestOperatorControl(bool allowOverride, int requestTimeoutSecs)
{
    ...
    int requestTimeoutSecsMin =
        SettingsManager::instance()->flyViewSettings()->requestControlTimeout()->cookedMin().toInt();
    int requestTimeoutSecsMax =
        SettingsManager::instance()->flyViewSettings()->requestControlTimeout()->cookedMax().toInt();
    if (requestTimeoutSecs >= requestTimeoutSecsMin && requestTimeoutSecs <= requestTimeoutSecsMax) {
        ...
        safeRequestTimeoutSecs =
            SettingsManager::instance()->flyViewSettings()->requestControlTimeout()->cookedDefaultValue().toInt();
    }
    ...
    sendMavCommandWithHandler(
        &handlerInfo, _defaultComponentId, MAV_CMD_REQUEST_OPERATOR_CONTROL,
        0,                      // System ID of GCS requesting control, 0 if it is this GCS
        1,                      // Action - 0: Release control, 1: Request control.
        allowOverride ? 1 : 0,  // Allow takeover - Enable automatic granting of ownership on request. 0: Ask current
                                // owner and reject request, 1: Allow automatic takeover.
        safeRequestTimeoutSecs  // Timeout in seconds before a request to a GCS to allow takeover is assumed to be
                                // rejected. This is used to display the timeout graphically on requestor and GCS in
                                // control.
    );

    // If this is a request we sent to other GCS, start timer so User can not keep sending requests until the current
    // timeout expires
...
void Vehicle::_requestOperatorControlAckHandler(void* resultHandlerData, int compId, const mavlink_command_ack_t& ack,
                                                MavCmdResultFailureCode_t failureCode)
{
...
{
    if (apmFirmware()) {
        sendMavCommand(defaultComponentId(), MAV_CMD_FLASH_BOOTLOADER,
                       true,        // show error
...
{
    if (apmFirmware()) {
        sendMavCommand(defaultComponentId(), MAV_CMD_DO_AUX_FUNCTION, true, APM::AUX_FUNC::MOTOR_INTERLOCK,
...
{
    m_statusTextHandler->resetAllMessages();
}

void Vehicle::resetErrorLevelMessages()
{
    m_statusTextHandler->resetErrorLevelMessages();
}
...
{
    return m_statusTextHandler->messageTypeNone();
}

bool Vehicle::messageTypeNormal() const
{
    return m_statusTextHandler->messageTypeNormal();
}

bool Vehicle::messageTypeWarning() const
{
    return m_statusTextHandler->messageTypeWarning();
}

bool Vehicle::messageTypeError() const
{
    return m_statusTextHandler->messageTypeError();
}

int Vehicle::messageCount() const
{
    return m_statusTextHandler->messageCount();
}

QString Vehicle::formattedMessages() const
{
    return m_statusTextHandler->formattedMessages();
}
...
void Vehicle::_textMessageReceived(MAV_COMPONENT componentid, MAV_SEVERITY severity, QString text, QString description)
{
    ...
    const bool px4Prearm = text.startsWith(QStringLiteral("preflight"), Qt::CaseInsensitive) &&
                           (severity >= MAV_SEVERITY::MAV_SEVERITY_CRITICAL);
    if (ardupilotPrearm || px4Prearm) {
        ...
        if (_noisySpokenPrearmMap.contains(text) &&
            _noisySpokenPrearmMap.value(text).msecsTo(QTime::currentTime()) < (10 * 1000)) {
            skipSpoken = true;
        } else {
...
void Vehicle::_createImageProtocolManager()
{
    ...
    (void) connect(_imageProtocolManager, &ImageProtocolManager::flowImageIndexChanged, this,
                   &Vehicle::flowImageIndexChanged);
    (void) connect(_imageProtocolManager, &ImageProtocolManager::imageReady, this,
                   [this](const QImage& image) { qgcApp()->qgcImageProvider()->setImage(image, _systemID); });
}
...
void Vehicle::_createMAVLinkEventManager()
{
    ...
    (void) connect(_eventManager.get(), &MAVLinkEventManager::statusTextMessageFromEvent, this,
                   &Vehicle::_onStatusTextFromEvent);
}
