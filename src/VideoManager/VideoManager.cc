...
void VideoManager::startVideoBackendInit()
{
    ...
    _backendInitFuture.then(this, [this](bool success) { _onBackendInitComplete(success); }).onCanceled(this, [this] {
...
void VideoManager::init(QQuickWindow* mainWindow)
{
    ...
    _mainWindow->scheduleRenderJob(
        QRunnable::create(
            [this] { QMetaObject::invokeMethod(this, &VideoManager::_initAfterQmlIsReady, Qt::QueuedConnection); }),
        ...
        switch (_initState.load()) {
            ...
            case InitState::Running:
                qCWarning(VideoManagerLog)
                    << "_initAfterQmlIsReady: unexpected state" << static_cast<int>(_initState.load());
                return;
        }
    }
...
void VideoManager::_onBackendInitComplete(bool success)
{
    ...
    switch (_initState.load()) {
        ...
        case InitState::QmlReady:
            ...
            return;
        default:
            qCWarning(VideoManagerLog) << "_onBackendInitComplete: unexpected state"
...
void VideoManager::_maybeWarnSoftwareDecoder()
{
    ...
    QGC::showAppMessage(
        tr("Video is using a software decoder (%1). Latency will be higher than hardware decode. "
...
bool VideoManager::isStreamSource() const
{
    static const QStringList videoSourceList = {
        ...
        VideoSettings::videoSourceSiyiZr10,
...
bool VideoManager::_updateSettings(VideoReceiver* receiver)
{
    ...
    const int rtpLatencyMs = static_cast<int>(
        std::min(_videoSettings->rtpJitterLatencyMs()->rawValue().toUInt(), static_cast<uint>(INT_MAX)));
    ...
    } else if (source == VideoSettings::videoSourceSiyiZr10) {
        ScreenToolsController::ensureSiyiRadioEthernet();
        settingsChanged |= _updateVideoUri(receiver, QString::fromLatin1(VideoSettings::siyiZr10RtspUrl));
        if (_videoSettings->rtspUrl()->rawValue().toString() != QLatin1String(VideoSettings::siyiZr10RtspUrl)) {
            _videoSettings->rtspUrl()->setRawValue(QString::fromLatin1(VideoSettings::siyiZr10RtspUrl));
        }
...
void VideoManager::_setActiveVehicle(Vehicle* vehicle)
{
    ...
    if (_activeVehicle) {
        ...
        if (cameraManager) {
            ...
            (void) disconnect(cameraManager, &QGCCameraManager::streamChanged, this,
...
void VideoManager::_checkUnipodEthernetGate()
{
    ...
    if (ScreenToolsController::isSiyiRadioEthernetReady()) {
        qCDebug(VideoManagerLog) << "SIYI radio ethernet ready after" << (_unipodEthernetGateTicks * 500)
                                 << "ms:" << ScreenToolsController::siyiRadioEthernetAddress();
        _disarmUnipodEthernetGate();
        _restartAllVideos();
        return;
    }
