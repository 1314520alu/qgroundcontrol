...
{
    if (!msg || !error || (error->domain != GST_STREAM_ERROR) || (error->code != GST_STREAM_ERROR_FORMAT) || !debug ||
        !g_strrstr(debug, "NAL unit type 50 not supported yet")) {
        return false;
    }
...
GstVideoReceiver::GstVideoReceiver(QObject* parent) : VideoReceiver(parent), _worker(new GstVideoWorker(this))
{
...
void GstVideoReceiver::start(uint32_t timeout)
{
    ...
    do {
        ...
        if (_teeProbeId == 0) {
            // _teeProbe updates _lastSourceFrameTime; without it the watchdog timer fires spuriously instead of
        ...
        sourceConfig.jitterBuffer = (_buffer < 0)
                                        ...
                                        : (_buffer == 0 ? GStreamer::SourceFactory::JitterBuffer::DropOnLatency
        ...
        sourceConfig.doRetransmission =
            (_rtpJitterLatencyMs >= 40) && (sourceConfig.jitterBuffer != GStreamer::SourceFactory::JitterBuffer::None);
        ...
        gst_bin_add_many(GST_BIN(_pipeline), _source, _tee, decoderQueue, _decoderValve, recorderQueue, _recorderValve,
                         nullptr);
    ...
    if (_pipeline) {
        ...
        if (bus) {
            ...
            if (!recordingValveClosed) {
                ...
                for (;;) {
                    const qint64 elapsedNs = (QDateTime::currentMSecsSinceEpoch() - startMs) * qint64(GST_MSECOND);
                    if (elapsedNs >= qint64(deadline))
                        break;
                    const GstClockTime remaining = GstClockTime(qint64(deadline) - elapsedNs);
                    GstMessage* msg = gst_bus_timed_pop_filtered(
                        bus, remaining, (GstMessageType) (GST_MESSAGE_EOS | GST_MESSAGE_ERROR | GST_MESSAGE_ELEMENT));
                    if (!msg)
                        break;
...
void GstVideoReceiver::_scheduleReconnect(const char* reason)
{
    ...
    QMetaObject::invokeMethod(
        this,
        [this, reason, reconnectTimeout, uri]() {
            ...
            qCInfo(GstVideoReceiverLog) << "Scheduling reconnect #" << attempts << "in" << delaySec << "s after"
                                        << reason << uri;
            QTimer::singleShot(delaySec * 1000, this, [this, epoch, attempts, reconnectTimeout, uri]() {
                ...
                if (uri.isEmpty() || pipelineUp)
                    return;  // pipeline already came back
...
GstElement* GstVideoReceiver::_makeFileSink(const QString& videoFile, FILE_FORMAT format)
{
    ...
    do {
        ...
        g_object_set(splitmux, "location", qPrintable(videoFile), "muxer-factory", _kFileMux[format], "max-size-time",
                     G_GUINT64_CONSTANT(0), "max-size-bytes", G_GUINT64_CONSTANT(0), "async-finalize", TRUE,
        ...
        if (format == FILE_FORMAT_MP4 || format == FILE_FORMAT_MOV) {
            GstStructure* muxerProps =
                gst_structure_new("properties", "faststart", G_TYPE_BOOLEAN, TRUE, "reserved-moov-update-period",
                                  G_TYPE_UINT64, G_GUINT64_CONSTANT(1000000000), nullptr);
...
{
    // FIXME: check for caps - if this is not video stream (and preferably - one of these which we have to support) then
    // simply skip it
        ...
        if (factory) {
            ...
            if (is_decoder) {
                ...
                qCDebug(GstVideoReceiverLog)
                    << "Decodebin3 selected codec:rank -" << pluginName << "/" << featureName << "-" << decoderKlass
...
bool GstVideoReceiver::_addVideoSink(GstPad* pad)
{
    ...
    do {
        ...
        if (gst_structure_get_int(structure, "video-orientation", &orientation) &&
            (orientation == GST_VIDEO_ORIENTATION_90R || orientation == GST_VIDEO_ORIENTATION_90L ||
             orientation == GST_VIDEO_ORIENTATION_UL_LR || orientation == GST_VIDEO_ORIENTATION_UR_LL)) {
...
void GstVideoReceiver::_shutdownDecodingBranch()
{
    ...
    if (_eosProbeId != 0 && _eosProbePad) {
        // Probe was installed on the source pad in _onNewSourcePad; remove from that exact pad — not from _decoder,
        // which may already be cleared above.
...
gboolean GstVideoReceiver::_onBusMessage(GstBus* /* bus */, GstMessage* msg, gpointer data)
{
    ...
    switch (GST_MESSAGE_TYPE(msg)) {
        case GST_MESSAGE_ERROR: {
            ...
            if (debug) {
                qCWarning(GstVideoReceiverLog) << "GStreamer debug:" << debug;
            ...
            if (error) {
                if (recoverableH265PaciError) {
                    ...
                    qCCritical(GstVideoReceiverLog) << "GStreamer error:" << error->message << "uri=" << pThis->uri();
                }
                g_clear_error(&error);
            }
        ...
        case GST_MESSAGE_WARNING: {
            ...
            qCWarning(GstVideoReceiverLog) << "GStreamer warning:" << (error ? error->message : "(no message)")
        ...
        case GST_MESSAGE_STREAM_COLLECTION: {
            ...
            for (guint i = 0; i < nStreams; ++i) {
                ...
                if (type & GST_STREAM_TYPE_VIDEO) {
                    selectedIds = g_list_append(selectedIds, g_strdup(gst_stream_get_stream_id(stream)));
                }
            }
            if (selectedIds) {
...
GstPadProbeReturn GstVideoReceiver::_keyframeWatch(GstPad* pad, GstPadProbeInfo* info, gpointer user_data)
{
    ...
    // set media file '0' offset to current timeline position - we don't want to touch other elements in the graph,
    // except these which are downstream!
    gst_pad_set_offset(pad, -static_cast<gint64>(buf->pts));
...
GstVideoWorker::GstVideoWorker(QObject* parent) : QThread(parent)
{
    qCDebug(GstVideoReceiverLog) << this;
}
