...
public slots:
    ...
    /// Display-branch `queue` depth. Low-latency stays at 2 (drop on backpressure).
    /// Buffered live holds enough compressed frames to absorb an HEVC IDR decode spike
    /// (~1 s GOP on UniPod MT11) without leaking the following P-frames.
    static int decoderQueueMaxBuffers(bool lowLatency);

    /// Clock-sync the live sink. Low-latency is unsynced. Buffered meters at the stream
    /// rate so a decoded-frame reservoir can hide IDR arrival; pair with unlimited max-lateness.
    static bool videoSinkSync(bool lowLatency);

    /// GstBaseSink processing-deadline (ns). Keep the 20 ms default: this value is added to
    /// the sink's latency query. Late-IDR drops are prevented by `videoSinkMaxLatenessNs`, not this.
    static quint64 videoSinkProcessingDeadlineNs(bool lowLatency);

    /// GstBaseSink max-lateness (ns). `-1` = never drop. Low-latency keeps the 20 ms default.
    static qint64 videoSinkMaxLatenessNs(bool lowLatency);
...
private:
    ...
    mutable QMutex
        _pipelineMutex;  // serializes _pipeline mutation (worker) vs read in _onBusMessage (streaming thread)
    ...
    std::atomic<int> _reconnectAttempts =
        0;  ///< Written on the streaming thread (_noteTeeFrame) and GUI thread (reconnect lambda); atomic.
    std::atomic<quint64> _reconnectEpoch = 0;  ///< Bumped on every stop() — pending singleShot lambdas check this
                                               ///< before firing, replacing an explicit cancel/pending-flag pair.
    ...
    GstPad* _eosProbePad =
        nullptr;  // ref-held: probe install pad, kept so removal targets the right pad regardless of _decoder lifecycle
    ...
    std::atomic<bool> _qosStatsDirty{
        false};  // set per QOS message (streaming thread), drained by _watchdog's 1 Hz emit

    static constexpr const char* _kFileMux[FILE_FORMAT_MAX + 1] = {"matroskamux", "qtmux", "mp4mux"};
};
